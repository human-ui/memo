...
ROOT = '/braintree/data2/active/users/qbilius/memo'
# SCRATCH = '/braintree/home/qbilius/computed'
DATA_DIR = ROOT  #os.path.join(ROOT, 'store')
DBPATH = '/braintree/home/qbilius/projects/memo/index.csv'
...
def store(args=None, dest=None, cwd=None, prefix=None):
    start = time.time()
    ...
    if dest is None:
        dest = setup_dest()
        shutil.copy2(os.path.abspath(args.script), dest)  # otherwise the script is already there
    ...
    rec = OrderedDict([('timestamp', timestamp),
                       ...
                       ('working dir', os.path.abspath(cwd)),
                       ('duration', np.nan),
                       ('version', version),
                       ('tag', args.tag),
